...
NS_LOG_COMPONENT_DEFINE("1905064_1");
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("TxRangeMultiplier","your given number will be used as multiplier with tx_range to set coverage area",mul);
    cmd.AddValue("datFileType","for variable vs performance metrics graphs.--1 for Nodes, 2 for Flows, 3 for Packets, 4 for coverage area",datType);
    cmd.Parse(argc, argv);


    double coverageArea = mul*tx_range;
    ...
    infos+="--coverageArea-"+std::to_string(coverageArea);
    ...
    for (uint32_t i = 0; i < Flows; i++)
    {
        ...
        OnOffHelper onOffHelper("ns3::TcpSocketFactory", InetSocketAddress(staRightInterfaces.GetAddress(index), port+i));
    ...
    if(datType==1)
    {
      std::ofstream dataFile("static_Nodes_vs_Throughput.dat", std::ios::app);
      dataFile << Nodes << "\t" << averageThroughput << "\n";
      dataFile.close();

      std::ofstream dataFile2("static_Nodes_vs_DeliveryRatio.dat", std::ios::app);
    ...
    {
      std::ofstream dataFile("static_Flows_vs_Throughput.dat", std::ios::app);
      dataFile << Flows << "\t" << averageThroughput << "\n";
      dataFile.close();

      std::ofstream dataFile2("static_Flows_vs_DeliveryRatio.dat", std::ios::app);
      dataFile2 << Flows << "\t" <<  1.0*received_packets/transmitted_packets<< "\n";
      dataFile2.close();
    }
    else if(datType==3)
    {
      std::ofstream dataFile("static_Packets_vs_Throughput.dat", std::ios::app);
      dataFile << PacketsPerSecond << "\t" << averageThroughput << "\n";
      dataFile.close();

      std::ofstream dataFile2("static_Packets_vs_DeliveryRatio.dat", std::ios::app);
      dataFile2 << PacketsPerSecond << "\t" <<  1.0*received_packets/transmitted_packets<< "\n";
      dataFile2.close();
    }
    else if(datType==4)
    {
      std::ofstream dataFile("static_CoverageArea_vs_Throughput.dat", std::ios::app);
      dataFile << coverageArea << "\t" << averageThroughput << "\n";
      dataFile.close();

      std::ofstream dataFile2("static_CoverageArea_vs_DeliveryRatio.dat", std::ios::app);
      dataFile2 << coverageArea << "\t" <<  1.0*received_packets/transmitted_packets<< "\n";
      dataFile2.close();
    }
